...
/**
 * @description 生成唯一key
 * @param {*} lastStr 上一次生成的key，用来和新生成的做比较
 * @returns 返回唯一key
 */
function createKey(lastStr) {
  const timestamp = Date.now().toString(36);
  const random = Math.random().toString(36).substring(2, 9);
  const key = timestamp + random;
  if (lastStr === key) {
    return createKey(lastStr);
  }
  return key;
}
...
Parser.prototype.onOpenTag = function (selfClose) {
  ...
  if (close) {
    ...
    node.children = [];
    const timestamp = createKey({ timeStamp: this.stack[this.stack.length - 1]?.timeStamp || 0 });
      ...
      for (let row = 1; row <= trList.length; row++) {
        ...
        for (let j = 0; j < trList[row - 1].children.length; j++) {
          ...
          if (td.name === 'td' || td.name === 'th') {
            ...
            style =
              (border ? `;border:${border}px ${borderstyle || 'solid'} ${bordercolor || 'gray'}` + (spacing ? '' : ';border-right:0;border-bottom:0') : '') +
